<?php 
    class AdminModel extends Model{
        function addLesson($tema, $descripcion, $url_video, $url_slide){
            $sentencia = $this->db->prepare("insert into lesson (tema_id, descripcion, video_url, slide_url) value(?, ?, ?, ?, ?)");
            $sentencia->execute(array($tema, $descripcion, $url_video, $url_slide));
            return $sentencia->fetchAll(PDO::FETCH_ASSOC);
        }
        
        function deleteLesson($lessonId){
            $sentencia = $this->db->prepare("delete from lesson where id=(?)");
            $sentencia->execute($lessonId);
            return $sentencia->fetchAll(PDO::FETCH_ASSOC);
        }
        function updateLesson($tema, $descripcion, $video_url, $slide_url, $lessonId){
            ...
            $sentencia->execute(array($tema, $descripcion, $video_url, $slide_url, $lessonId[0]));
            return $sentencia->fetchAll(PDO::FETCH_ASSOC);
        }
